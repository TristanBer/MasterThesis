...
MODEL_TAG = "DualBranch_R2Plus1D"
RESULTS_DIR = "/workspace/results_DualBranch_R2Plus1D_5fold"
...
def train_one_fold(fold_idx, train_indices, val_indices,
    ...
    """
    Trains and evaluates a single fold using the identical two-stage protocol
    as the original single-split script. Returns the final-checkpoint accuracy,
    macro-F1, and the validation predictions/labels for out-of-fold pooling.
    """
    print(f"\n############## FOLD {fold_idx + 1}/{N_FOLDS} ##############")

    # Per-fold data-leakage verification
    ...
    train_loader = DataLoader(train_db, batch_size=BATCH_SIZE, shuffle=True, num_workers=12, pin_memory=True, drop_last=True)
    val_loader = DataLoader(val_db, batch_size=BATCH_SIZE, shuffle=False, num_workers=12, pin_memory=True)

    # Class weights are RECOMPUTED for this fold's training subset only
    class_weights, class_weights_meta = compute_class_weights(full_dataset_train, train_indices, device)

    # A FRESH model is instantiated for every fold (no weight leakage across folds)
    num_classes = len(full_dataset_train.class_names)
    model = FineGrainedDualStreamModel(num_classes=num_classes, freeze_backbone=True, dropout_p=0.5).to(device)
    criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.1)

    history = {"train_loss": [], "val_loss": [], "train_acc": [], "val_acc": []}
    ckpt_path = os.path.join(RESULTS_DIR, f"DualBranch_R2Plus1D_best_fold{fold_idx}.pth")

    # --- STAGE 1: Train only the FC head ---
    print(f"\n=== [Fold {fold_idx + 1}] STAGE 1: Training head only for {STAGE1_EPOCHS} epochs ===")
    optimizer = optim.Adam(model.classifier.parameters(), lr=LEARNING_RATE_S1)
    best_val_acc = 0.0
    ...
    # --- FINAL EVALUATION for this fold (best checkpoint) ---
    ...
    macro_f1 = f1_score(final_labels, final_preds, average='macro',
                        labels=label_ids, zero_division=0) * 100
    weighted_f1 = f1_score(final_labels, final_preds, average='weighted',
                           labels=label_ids, zero_division=0) * 100
    print(f"[Fold {fold_idx + 1}] Final Val Accuracy: {final_acc:.2f}% | "
          f"Macro-F1: {macro_f1:.2f}% | Weighted-F1: {weighted_f1:.2f}%")

    # Per-fold classification report (dict form -> per-class precision/recall/F1/support)
    report_dict = classification_report(final_labels, final_preds,
                                        target_names=class_names, labels=label_ids,
                                        zero_division=0, output_dict=True)
    print("\nPer-class report:")
    print(classification_report(final_labels, final_preds, target_names=class_names,
                                labels=label_ids, zero_division=0))

    # Per-fold confusion matrix (rows = actual, cols = predicted)
    cm = confusion_matrix(final_labels, final_preds, labels=label_ids)

    # ---- PERSIST EVERYTHING FOR THIS FOLD ----
    fold_payload = {
        "model": MODEL_TAG,
        "fold": fold_idx,
        "hyperparameters": {
    ...
    # Per-fold confusion matrix as plain CSV (easy to drop into the thesis)
    ...
    # Per-fold per-class metrics as CSV
    ...
    plt.title(f"R2+1D Accuracy (Fold {fold_idx + 1})")
    plt.xlabel("Epochs"); plt.ylabel("Accuracy (%)"); plt.ylim(0, 100); plt.legend(); plt.grid(True)
    plt.subplot(1, 2, 2)
    plt.plot(epochs_range, history["train_loss"], label="Train Loss", marker='o')
    plt.plot(epochs_range, history["val_loss"], label="Val Loss", marker='o')
    plt.axvline(x=STAGE1_EPOCHS, color='gray', linestyle='--', label='Unfreeze point')
    plt.title(f"R2+1D Loss (Fold {fold_idx + 1})")
    plt.xlabel("Epochs"); plt.ylabel("Loss"); plt.legend(); plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"R2Plus1D_learning_curve_fold{fold_idx}.png"), dpi=300)
    plt.close()

    # Per-fold confusion matrix figure
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=class_names, yticklabels=class_names)
    plt.ylabel('Actual setting action'); plt.xlabel('Predicted setting action')
    plt.title(f'Confusion Matrix - Fold {fold_idx + 1} (R2+1D)')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"confusion_matrix_R2Plus1D_fold{fold_idx}.png"), dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    # Deterministic 5-fold generator (no shuffle => reproducible folds)
    ...
    fold_macro_f1 = []
    fold_weighted_f1 = []
    oof_preds = []   # out-of-fold predictions: every clip appears exactly once
    oof_labels = []

    # ---- LOOP OVER ALL FIVE FOLDS (the key change) ----
    ...
    for i in range(N_FOLDS):
        print(f"  Fold {i + 1}: Acc = {fold_accuracies[i]:.2f}% | "
              f"Macro-F1 = {fold_macro_f1[i]:.2f}% | Weighted-F1 = {fold_weighted_f1[i]:.2f}%")
    print("---------------------------------------------------")
    ...
    # ===== POOLED OUT-OF-FOLD REPORT (full dataset, each clip judged once, OOD) =====
    print("\n========= POOLED OUT-OF-FOLD CLASSIFICATION REPORT =========")
    oof_report_text = classification_report(oof_labels, oof_preds, target_names=class_names,
                                            labels=label_ids, zero_division=0)
    ...
    # ---- WRITE COMBINED SUMMARY JSON (everything needed to write the chapter) ----
    summary = {
        "model": MODEL_TAG,
        ...
        "class_names": class_names,
        "per_fold": {
            "accuracy": fold_accuracies.tolist(),
            "macro_f1": fold_macro_f1.tolist(),
    ...
    # ---- WRITE PER-FOLD SUMMARY CSV (drop straight into the thesis table) ----
    with open(os.path.join(RESULTS_DIR, "summary_per_fold.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["fold", "accuracy", "macro_f1", "weighted_f1"])
        for i in range(N_FOLDS):
            w.writerow([i + 1, f"{fold_accuracies[i]:.2f}", f"{fold_macro_f1[i]:.2f}",
                        f"{fold_weighted_f1[i]:.2f}"])
        w.writerow(["mean", f"{fold_accuracies.mean():.2f}", f"{fold_macro_f1.mean():.2f}",
                    f"{fold_weighted_f1.mean():.2f}"])
        w.writerow(["std", f"{fold_accuracies.std():.2f}", f"{fold_macro_f1.std():.2f}",
                    f"{fold_weighted_f1.std():.2f}"])

    # ---- WRITE POOLED OOF PER-CLASS METRICS CSV ----
    ...
    # ---- WRITE POOLED OOF CONFUSION MATRIX CSV ----
    ...
    # ---- SAVE RAW OOF PREDICTIONS (recompute any metric later, no retraining) ----
    ...
    # ---- SAVE THE PLAIN-TEXT OOF REPORT TOO ----
    ...
    plt.title('Pooled Out-of-Fold Confusion Matrix (5-fold CV)')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "confusion_matrix_R2Plus1D_oof_5fold.png"), dpi=300)
    plt.close()

    print(f"\nAll metrics, reports, matrices and raw predictions saved to: {RESULTS_DIR}")
    print("Key files: SUMMARY_5fold.json, summary_per_fold.csv, oof_per_class_metrics.csv,")
    print("           oof_confusion_matrix.csv, oof_predictions.csv, fold*_results.json")
