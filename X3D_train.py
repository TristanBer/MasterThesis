...
if __name__ == '__main__':
    ...
    model = VolleyballX3DModel(num_classes=num_classes, freeze_backbone=True, dropout_p=0.5).to(device)
    ...
    for epoch in range(STAGE1_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), "/workspace/X3D_best.pth")
    ...
    plt.title("X3D Training and Validation Accuracy")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy (%)")
    plt.ylim(0, 100)
    plt.legend()
    ...
    plt.axvline(x=STAGE1_EPOCHS, color='gray', linestyle='--', label='Unfreeze point')
    plt.title("X3D Training and Validation Loss")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("/workspace/X3D_learning_curve_first_run.png", dpi=300)
    print("\nGraph saved as 'X3D_learning_curve_first_run.png'")

    # --- 9. CONFUSION MATRIX ---
    print("Create Confusion Matrix for X3D...")
    ...
    plt.tight_layout()
    plt.savefig("/workspace/confusion_matrix_X3D_first_run.png", dpi=300)
    print("Confusion Matrix saved as 'confusion_matrix_X3D_first_run.png'")