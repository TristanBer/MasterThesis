...
if __name__ == '__main__':
    ...
    model = VolleyballVideoSwinModel(num_classes=num_classes, freeze_backbone=True, dropout_p=0.5).to(device)
    ...
    for epoch in range(STAGE1_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), "/workspace/VideoSwin_best.pth")
    ...
    model.load_state_dict(torch.load("/workspace/VideoSwin_best.pth", map_location=device, weights_only=True))
    ...
    plt.title("VideoSwin Training and Validation Accuracy")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy (%)")
    plt.ylim(0, 100)
    plt.legend()
    ...
    plt.axvline(x=STAGE1_EPOCHS, color='gray', linestyle='--', label='Unfreeze point')
    plt.title("VideoSwin Training and Validation Loss")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("/workspace/VideoSwin_learning_curve_first_run.png", dpi=300)
    print("\nGraph saved as 'VideoSwin_learning_curve_first_run.png'")

    # --- 9. CONFUSION MATRIX ---
    print("Erstelle Confusion Matrix für VideoSwin...")
    ...
    plt.tight_layout()
    plt.savefig("/workspace/confusion_matrix_VideoSwin_first_run.png", dpi=300)
    print("Confusion Matrix als 'confusion_matrix_VideoSwin_first_run.png' gespeichert!")