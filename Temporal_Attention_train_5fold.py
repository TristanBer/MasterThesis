...
MODEL_TAG = "R2Plus1D_TemporalAttn"
RESULTS_DIR = "/workspace/results_TemporalAttn_5fold"
...
def train_one_fold(fold_idx, train_indices, val_indices,
    ...
    # drop_last=True added here to prevent 1-sample batch normalization crashes
    ...
    # Initialize the new Temporal Attention Model
    model = R2Plus1DTemporalAttention(num_classes=num_classes, freeze_backbone=True, dropout_p=0.5).to(device)
    criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.1)

    history = {"train_loss": [], "val_loss": [], "train_acc": [], "val_acc": []}
    ckpt_path = os.path.join(RESULTS_DIR, f"TemporalAttn_best_fold{fold_idx}.pth")

    # --- STAGE 1: Train Head + Attention Network ---
    print(f"\n=== [Fold {fold_idx + 1}] STAGE 1: Training head & attention only for {STAGE1_EPOCHS} epochs ===")

    # Explicitly target both the classifier AND the new attention mechanism
    trainable_params = list(model.attention_net.parameters()) + list(model.classifier.parameters())
    optimizer = optim.Adam(trainable_params, lr=LEARNING_RATE_S1)
    ...
    fold_payload = {
        ...
        "train_matches": train_matches, "val_matches": val_matches,
        "n_train_clips": int(len(train_indices)), "n_val_clips": int(len(val_indices)),
        "class_names": class_names, "class_weights": class_weights_meta,
        "final_val_accuracy": final_acc, "macro_f1": macro_f1, "weighted_f1": weighted_f1,
        "best_val_acc_during_training": best_val_acc, "classification_report": report_dict,
        "confusion_matrix": cm.tolist(), "history": history,
        "val_predictions": final_preds, "val_labels": final_labels,
    }
    ...
    with open(os.path.join(RESULTS_DIR, f"fold{fold_idx}_confusion_matrix.csv"), "w", newline="") as f:
        ...
        for r, name in enumerate(class_names): w.writerow([name] + cm[r].tolist())
    ...
    with open(os.path.join(RESULTS_DIR, f"fold{fold_idx}_per_class_metrics.csv"), "w", newline="") as f:
        ...
        for name in class_names:
            ...
            w.writerow(
                [name, row.get("precision", 0), row.get("recall", 0), row.get("f1-score", 0), row.get("support", 0)])

    plt.figure(figsize=(12, 5))
    plt.subplot(1, 2, 1)
    plt.plot(range(1, STAGE1_EPOCHS + STAGE2_EPOCHS + 1), history["train_acc"], label="Train Accuracy", marker='o')
    plt.plot(range(1, STAGE1_EPOCHS + STAGE2_EPOCHS + 1), history["val_acc"], label="Val Accuracy", marker='o')
    ...
    plt.grid(True)
    plt.subplot(1, 2, 2)
    plt.plot(range(1, STAGE1_EPOCHS + STAGE2_EPOCHS + 1), history["train_loss"], label="Train Loss", marker='o')
    plt.plot(range(1, STAGE1_EPOCHS + STAGE2_EPOCHS + 1), history["val_loss"], label="Val Loss", marker='o')
    plt.axvline(x=STAGE1_EPOCHS, color='gray', linestyle='--', label='Unfreeze point')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"learning_curve_fold{fold_idx}.png"), dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"confusion_matrix_fold{fold_idx}.png"), dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    groups = [os.path.basename(p).split('_set_')[0].split('_frame_')[0] for p in full_dataset_train.video_files]
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(
            sgkf.split(np.zeros(len(full_dataset_train)), full_dataset_train.labels, groups=groups)):
        final_acc, macro_f1, weighted_f1, fold_preds, fold_labels = train_one_fold(
            fold_idx, train_indices, val_indices, full_dataset_train, full_dataset_val, groups, device)
    ...
    summary = {
        "model": MODEL_TAG, "timestamp": datetime.datetime.now().isoformat(timespec="seconds"), "n_folds": N_FOLDS,
        "class_names": class_names,
        "per_fold": {"accuracy": fold_accuracies.tolist(), "macro_f1": fold_macro_f1.tolist(),
        ...
        "pooled_oof": {"accuracy": oof_accuracy, "macro_f1": oof_macro_f1, "classification_report": oof_report_dict,
    ...
    with open(os.path.join(RESULTS_DIR, "summary_per_fold.csv"), "w", newline="") as f:
        ...
        for i in range(N_FOLDS): w.writerow(
            [i + 1, f"{fold_accuracies[i]:.2f}", f"{fold_macro_f1[i]:.2f}", f"{fold_weighted_f1[i]:.2f}"])
        w.writerow(
    ...
    with open(os.path.join(RESULTS_DIR, "oof_confusion_matrix.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["actual\\predicted"] + class_names)
        for r, name in enumerate(class_names): w.writerow([name] + oof_cm[r].tolist())
    ...
    with open(os.path.join(RESULTS_DIR, "oof_predictions.csv"), "w", newline="") as f:
        ...
        for i, (t, p) in enumerate(zip(oof_labels, oof_preds)): w