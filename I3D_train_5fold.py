...
MODEL_TAG = "I3D"
RESULTS_DIR = "/workspace/results_i3d_5fold"
...
def train_one_fold(fold_idx, train_indices, val_indices,
    ...
    """
    Trains and evaluates a single fold. Returns the final-checkpoint accuracy,
    ...
    ckpt_path = f"/workspace/i3d_best_fold{fold_idx}.pth"
    ...
    # Keeping CosineAnnealingLR for I3D as defined in the original configuration
    ...
    plt.title(f"I3D Accuracy (Fold {fold_idx + 1})")
    plt.xlabel("Epochs");
    plt.ylabel("Accuracy (%)");
    plt.ylim(0, 100);
    plt.legend();
    ...
    plt.axvline(x=STAGE1_EPOCHS, color='gray', linestyle='--', label='Unfreeze point')
    plt.title(f"I3D Loss (Fold {fold_idx + 1})")
    plt.xlabel("Epochs");
    plt.ylabel("Loss");
    plt.legend();
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"i3d_learning_curve_fold{fold_idx}.png"), dpi=300)
    plt.close()
    ...
    plt.title(f'Confusion Matrix - Fold {fold_idx + 1} (I3D)')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"confusion_matrix_i3d_fold{fold_idx}.png"), dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomRotation(degrees=10),  # Kept I3D specific rotation
        transforms.ToTensor(),
    ...
    # ---- SAVE THE PLAIN-TEXT OOF REPORT ----
    ...
    plt.ylabel('Actual setting action')
    plt.xlabel('Predicted setting action')
    plt.title('Pooled Out-of-Fold Confusion Matrix (5-fold CV) - I3D')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "confusion_matrix_i3d_oof_5fold.png"), dpi=300)
    plt.close()
