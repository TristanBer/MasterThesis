...
MODEL_TAG = "CNN_BiLSTM"
RESULTS_DIR = "/workspace/results_CNN_BiLSTM_5fold"
...
def train_one_fold(fold_idx, train_indices, val_indices,
    ...
    """
    Trains and evaluates a single fold using the two-stage protocol.
    Returns the final-checkpoint accuracy, macro-F1, and the validation predictions/labels.
    """
    ...
    # A FRESH model is instantiated for every fold
    ...
    ckpt_path = f"/workspace/CNN_BiLSTM_best_fold{fold_idx}.pth"
    ...
    # CNN_BiLSTM specific parameter grouping
    ...
    plt.title(f"CNN+BiLSTM Accuracy (Fold {fold_idx + 1})")
    plt.xlabel("Epochs");
    plt.ylabel("Accuracy (%)");
    plt.ylim(0, 100);
    plt.legend();
    ...
    plt.axvline(x=STAGE1_EPOCHS, color='gray', linestyle='--', label='Unfreeze point')
    plt.title(f"CNN+BiLSTM Loss (Fold {fold_idx + 1})")
    plt.xlabel("Epochs");
    plt.ylabel("Loss");
    plt.legend();
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"CNN_BiLSTM_learning_curve_fold{fold_idx}.png"), dpi=300)
    plt.close()
    ...
    plt.title(f'Confusion Matrix - Fold {fold_idx + 1} (CNN+BiLSTM)')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"confusion_matrix_CNN_BiLSTM_fold{fold_idx}.png"), dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    plt.ylabel('Actual setting action')
    plt.xlabel('Predicted setting action')
    plt.title('Pooled Out-of-Fold Confusion Matrix (5-fold CV) - CNN+BiLSTM')
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "confusion_matrix_CNN_BiLSTM_oof_5fold.png"), dpi=300)
    plt.close()
